...
if (globalVar.page == 'room_reservation_add') {
    ...
    $('#room_type_id').change(function(e) {
        $('#room_num').html('');
        $('#rooms_list').html('');
        var post_data = { room_type_id: $(this).val(), checkin_date:  $("#check_in_date_my").val() };
        ...
        $.ajax({
            type: 'GET',
            url: '/api/get-room-price',
            data: { room_type_id: $(this).val(), checkin_date:  $("#check_in_date_my").val() },
            datatype: "JSON",
            success: function(response) {
                console.log(response);
                $("#booking_payment").val(response.final_price);
            },
            error: function() {
                console.log('error');
            }
    ...
    $('#checkin_room_type_id').change(function(e) {
        $('#room_num').html('');
        $('#rooms_list').html('');
        var post_data = { room_type_id: $(this).val(), checkin_date:  $("#check_in_date_my").val() };
        globalFunc.ajaxCall('api/checkin-get-room-num-list', post_data, 'POST', globalFunc.before, globalFunc.listOfRooms, globalFunc.error, globalFunc.complete);
    ...
    globalFunc.listOfRooms = function(data) {
        ...
        var dirty_rooms = data.dirty_rooms;
        console.log(dirty_rooms);
        if (Object.keys(data.rooms).length > 0) {
            ...
            $.each(data.rooms, function(index, val) {
                ...
                if (dirty_rooms[val] != undefined) {
                    statusBtn = '<span class="btn btn-xs" style="background-color: brown;border-color:brown;color:white;">Room Readiness</span>';
                    checkbox = '<input name="room_num_booked[]"  type="checkbox" value="' + val + '" disabled></td>';
                }
    ...
    $(document).on('click', '.room_checkbox', function() {
        ...
        $("#arrival_room_num").val(globalVar.room_nums);
        ...
        if(parseInt(room_ids.length) == parseInt(no_of_room))
        {
            $("#arrivals_btn").attr("disabled", false);
        }
        else{
