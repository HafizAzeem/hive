...
@endphp
<div class="">
{{ Form::open(array('url'=>route('save-room-order'),'id'=>"food-order-form", 'class'=>"form-horizontal form-label-left",'files'=>true)) }}
                    ...
                    <div class="col-md-4 col-sm-4 col-xs-12">
                      <label class="control-label"> Room Number </label>
                        <select id="table_num" name="table_num" class="form-control col-md-12 roomnumberchange" required>
                            ...
                            @foreach($skillsnew as $item)
                                <option value="{{ $item->room_num }}">{{ $item->room_num }}</option>
                            @endforeach
                        </select>
                      {{--Form::select('table_num',getRoomNums(),null,['class'=>"form-control col-md-6 col-xs-12", "id"=>"table_num", "placeholder"=>lang_trans('ph_select'), "required"=>true])--}}
                    </div>
                    <div class="col-md-4 col-sm-4 col-xs-12">
...
<script>
    $(document).on('change','.roomnumberchange',function(e){
        ...
        $.ajax({
            type:'POST',
            url:"{{ route('getrooomdatacustomer') }}",
            data:{"_token": "{{ csrf_token() }}",roomno:roomno},
