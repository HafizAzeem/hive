                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 col-sm-4 col-xs-12">
                            ...
                            <select class="form-control col-md-12 col-xs-12" id='listed_rooms' name="listed_rooms[]" multiple>
                                ...
                                foreach($customer_room_data as $key => $room_dt){
                                    if($key == 0){
                                    $arrBookedRooms = explode(",", str_replace("[", "", str_replace("]", "", $room_dt->room_num)));
                                    for($i=0; $i<count($arrBookedRooms); $i++){
                                        echo '<option class="room_options" id="'.$room_dt->id.'" value="'. $arrBookedRooms[$i].'" data-room="'.$room_dt->room_num.'" data-per-room-price = "'.$room_dt->per_room_price.'" data-booking-payment="'.$room_dt->booking_payment.'" data-advance-payment="'.$room_dt->advance_payment.'" data-sec-advance-payment="'.$room_dt->sec_advance_payment.'">'.$arrBookedRooms[$i].'</option>';
                                    }
                                    }
                                }
                                ?> 
...
<div class="col-md-2">
                    ...
                    <select class="form-control" name="payment_remark[]" disabled>
                        ...
                        <label class="control-label"> Payment Mode</label>                            
                         {{Form::select('payment_mode[]',$payment_mode_list,$pp->mode,['data-id' => $pp->id, 'class'=>"form-control col-md-6 col-xs-12","id"=>"payment2", "placeholder"=>"--Select"])}}
